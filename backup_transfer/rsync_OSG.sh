...
ssh agladstein@${HOST_NAME} find ${OSG_DIR}*/outputs -maxdepth 1 -type f -name "final_results.txt" | cut -d "/" -f5 | cut -d "_" -f4- | xargs -n 1 -I % echo "mkdir -p ${ATMO_DIR}/macsswig_simsaj_$VERSION_% && rsync -avz agladstein@${HOST_NAME}:${OSG_DIR}_%/outputs/final_results.txt ${ATMO_DIR}/macsswig_simsaj_$VERSION_%/" | bash
