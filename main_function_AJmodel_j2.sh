...
if [ "$SYSTEM" == "ocelote" ]; then
    ...
    for i in {1..3}
    do
	    QUE=standard JNUM=1-2500 NODE=1 CORE=1 MEM=6 PYTHONENV=${VIRTUAL_ENV}/bin/python MODEL=${i} OUT_PATH=${OUT} j2 template.pbs.j2 >model${i}_${SYSTEM}_standard.pbs
	    QUE=qualified JNUM=1-2500 NODE=1 CORE=1 MEM=6 PYTHONENV=${VIRTUAL_ENV}/bin/python MODEL=${i} OUT_PATH=${OUT} j2 template.pbs.j2 >model${i}_${SYSTEM}_qualified.pbs
	    QUE=windfall JNUM=1-5000 NODE=1 CORE=1 MEM=6 PYTHONENV=${VIRTUAL_ENV}/bin/python MODEL=${i} OUT_PATH=${OUT} j2 template.pbs.j2 >model${i}_${SYSTEM}_windfall.pbs
    done
else
