...
def main():
    ...
    for chr_number in range(1,chr+1):
        ...
        profile = p.cpu_times(),p.memory_full_info()
        fprof.write('GERMLINE\t'+str(profile)+'\n')
...
#			rmmatch=Popen.wait(Popen('rm '+str(filenameout)+'.match',shell=True))
            ...
            profile = p.cpu_times(),p.memory_full_info()
            fprof.write('read_germline_lists\t'+str(profile)+'\n')
            ...
            for pa in pairs:
                IBDlengths_num.append(len(pa))
                if len(pa)<1:
                    pa.append(0)
                IBDlengths_mean.append(np.mean(pa))
                IBDlengths_median.append(np.median(pa))
                IBDlengths_var.append(np.var(pa))
