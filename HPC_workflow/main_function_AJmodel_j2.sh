...
#	PYTHONMOD=${PYTHONMOD} QUE=windfall JNUM=1-2000 NODE=1 CORE=1 MEM=6 PYTHONENV=${PBS_ENV}/bin/python MODEL=${MODEL} OUT_PATH=${OUT} j2 template.pbs.j2 >model${MODEL}_${SYSTEM}_windfall.pbs
	...
	PYTHONMOD=${PYTHONMOD} QUE=windfall JNUM=1-10 TIME=1 NODE=1 CORE=1 MEM=${ICE_MEM} PYTHONENV=${VIRTUAL_ENV}/bin/python MODEL=${MODEL} JOBTYPE=${SYSTEM}_only OUT_PATH=${OUT} j2 chain_pbs.j2 >PBS/chain_model${MODEL}_${SYSTEM}_windfall.pbs
	PYTHONMOD=${PYTHONMOD} QUE=windfall JNUM=1-10 TIME=1 NODE=1 CORE=1 MEM=${ICE_MEM} PYTHONENV=${VIRTUAL_ENV}/bin/python MODEL=${MODEL} JOBTYPE=${SYSTEM}_only OUT_PATH=${OUT} j2 macsargs_pbs.j2 >PBS/macsargs_model${MODEL}_${SYSTEM}_windfall.pbs
