...
from simulation import def_params_M3, run_sim_M3
...
def main(arguments):
    ...
    if arguments[5] == 'prior':
        param_model = def_params_M3.param_sim_asc_rand()
    if arguments[5] == 'min':
        param_model = def_params_M3.param_sim_asc_min()
    if arguments[5] == 'max':
        param_model = def_params_M3.param_sim_asc_max()
    ...
    while flag_sim == False:
        ...
        sim = run_sim_M3.run_sim(parameters, case, length, chr_number, total, total_naf, total_nas, total_neu, nJ, nM, nEA, nWA, seed_option)
